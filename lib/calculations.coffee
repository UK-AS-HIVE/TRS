...
(exports ? @).currentlyAllocated = (myRank, context) ->
  ...
  if (myRank == '')
    ...
    amounts = TRS.FacultyAllocations.find({semester: context.hash.semester || @semester, department: context.hash.department || @department}, {fields: {payAmount: 1, rank: 1, pay_amount: 1, lines: 1, name: 1, department: 1, semester: 1}}).forEach (doc) ->
